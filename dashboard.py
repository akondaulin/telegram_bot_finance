...
def get_monthly_costs():
    try:
        conn = psycopg2.connect(dbname=dbname, user=user, password=password, host=host, port=port)
        cur = conn.cursor()
        sql = "SELECT date, SUM(amount) FROM finance.costs WHERE date >= DATE_TRUNC('month', CURRENT_DATE) GROUP BY date ORDER BY date"
        cur.execute(sql)
        rows = cur.fetchall()
        cur.close()
        conn.close()
...
def show_weekly_stats(bot, message):
...
def show_monthly_stats(bot, message):
    rows = get_monthly_costs()
    if rows:
        dates = [row[0] for row in rows]
        amounts = [row[1] for row in rows]
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(dates, amounts, marker='o')
        plt.title('График расходов за месяц')
        plt.xlabel('Дата')
        plt.ylabel('Сумма расходов')
        plt.grid(True)
        plt.xticks(rotation=45)
        ...
        bot.send_photo(message.chat.id, buffer, caption='График расходов за месяц')
    else:
